...
options:
  state:
    description:
    - If C(present), will verify Chassis Connection Policy is present and will create if needed.
    - If C(absent), will verify Chassis Connection Policy is absent and will delete if needed.
    choices: [present, absent]
    default: present
  name:
    description:
    - The name assigned to the Chassis Connection Policy.
    - The Chassis Connection Policy name is case sensitive.
    - This name can be between 1 and 16 alphanumeric characters.
    - "You cannot use spaces or any special characters other than - (hyphen), \"_\" (underscore), : (colon), and . (period)."
    - You cannot change this name after the Chassis Connection Policy is created.
    required: yes
  description:
    description:
    - A description of the Chassis Connection Policy.
...
- name: Add Chassis Connection Policy
  ucs_chassis_connection:
    ...
    password: password
    name: s3260_single
    sioc_connectivity: 'single-server-single-sioc'
...
def main():
	...
	argument_spec.update(
		...
		sioc_connectivity=dict(type='str', default='single-server-single-sioc', choices=['single-server-single-sioc', 'single-server-dual-sioc']),
	...
	try:
		...
		if module.params['state'] == 'absent':
			...
			if mo_exists:
				...
				kwargs['serverSiocConnectivity'] = module.params['sioc_connectivity']
			...
			if not props_match:
				if not module.check_mode:
					...
					server_sioc_connectivity=module.params['sioc_connectivity'],
					)
