...
options:
  state:
    description:
    - If C(present), will verify Chassis Firmware Policy is present and will create if needed.
    - If C(absent), will verify Chassis Firmware Policy is absent and will delete if needed.
    choices: [present, absent]
    default: present
  name:
    description:
    - The name assigned to the Chassis Firmware Policy.
    - The Chassis Firmware Policy name is case sensitive.
    - This name can be between 1 and 16 alphanumeric characters.
    - "You cannot use spaces or any special characters other than - (hyphen), \"_\" (underscore), : (colon), and . (period)."
    - You cannot change this name after the Chassis Firmware Policy is created.
    required: yes
  description:
    description:
    - A description of the Chassis Firmware Package Policy.
  ...
  chassis_package:
    description:
    - The version of the firmware package that will be applied to the Cisco UCS S3260 chassis.
    default: ''
  service_pack:
    description:
...
EXAMPLES = r'''
- name: Add Chassis Firmware Package
  ucs_chassis_fw:
    hostname: 172.16.143.150
    username: admin
    password: password
    name: s3260_fw32
    chassis_package: '3.2(2b)C'
- name: Remove Chassis Firmware Package
  ucs_chassis_fw:
    hostname: 172.16.143.150
    username: admin
    password: password
...
def main():
	...
	argument_spec.update(
		...
		excluded_components=dict(type='str', default='local-disk', choices=['iocard', 'chassis-board-controller', 'cmc', 'local-disk', 'sas-expander']),
	...
	try:
		mo_exists = False
		props_match = False
		# dn is org-root/fw-chassis-pack-<name> for chassis firmware packages
		dn_base = 'org-root'
		dn = dn_base + '/fw-chassis-pack-' + module.params['name']
		...
		if module.params['state'] == 'absent':
			...
			if not props_match:
				if not module.check_mode:
					...
					mo = FirmwareChassisPack(
					parent_mo_or_dn=dn_base,
					name=module.params['name'],
					descr=module.params['description'],
					chassis_bundle_version=module.params['chassis_package'],
					service_pack_bundle_version=module.params['service_pack'],
					)
					#for component in module.params['excluded_components']:
					mo_1 = FirmwareExcludeChassisComponent(
					parent_mo_or_dn=mo,
					chassis_component=module.params['excluded_components']
					)
